...
// Sends a command byte to the OLED via SPI.
void writeCommand(unsigned char c) {
    unsigned long ulDummy;

    GPIOPinWrite(GPIOA0_BASE, 0x80, 0); // Data/Command select (command). Pin 62.
    GPIOPinWrite(GPIOA0_BASE, 0x40, 0); // Chip select enable. Pin 61.
    ...
    GPIOPinWrite(GPIOA0_BASE, 0x40, 0x40); // Chip select disable. Pin 61.
}
...
void writeData(unsigned char c) {
    ...
    GPIOPinWrite(GPIOA0_BASE, 0x80, 0x80); // Data/Command select (data). Pin 62.
    GPIOPinWrite(GPIOA0_BASE, 0x40, 0); // Chip select enable. Pin 61.
